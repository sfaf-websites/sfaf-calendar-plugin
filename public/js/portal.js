...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // After richText, so an editor started there is one this can mark.
        run('requiredMarks', initRequiredMarks);
        ...
        run('fixedSeriesImageFilter', initFixedSeriesImageFilter);
     ...
     *
     * WHAT THE SERVER ALREADY RENDERED is a working control with no script at
     * all: a file input and a list of pictures, inside a <details>. What this
     * adds is the one part a browser has to do, which is putting the chosen
     * picture INTO the editor. Without it somebody can still upload a file and
     * still see what is in the folder; they just cannot place it.
     *
     * THE MARKUP IS BUILT HERE, FROM A URL. The endpoint answers with an
     * address and nothing else, so it cannot be used to put arbitrary HTML into
     * a description by replying with it. The <img> this writes carries no
     * width, no style and no alignment: the stylesheet gives every picture in a
     * description the full width of the column and 14px corners, in the editor
     * frame and on the event page from the same rule, so a size written here
     * would be a second answer to a question already settled.
     *
     * EVERY ENDPOINT DETAIL COMES OFF THE ELEMENT, the way the fetch control on
     * the pending screen does it. Nothing is read from a global.
     * ------------------------------------------------------------------ */
    function initDescImages() {
        var box = document.querySelector('[data-uc-desc-images]');
        if (!box) { return; }

        var editorId = box.getAttribute('data-uc-editor') || 'uc-description';
        var status   = box.querySelector('[data-uc-desc-status]');
        ...
        /*
         * INTO TinyMCE IF IT IS RUNNING, INTO THE TEXTAREA IF IT IS NOT.
         * The editor may be the rich one or the plain fallback, and a paste
         * into the wrong one is a picture that silently does not save.
         */
        ...
        // Pick one already uploaded.
        var picks = box.querySelectorAll('[data-uc-desc-pick]');
        for (var i = 0; i < picks.length; i++) {
            picks[i].addEventListener('click', function () {
                if (insert(this.getAttribute('data-uc-desc-pick'), this.getAttribute('data-uc-desc-alt'))) {
                    say('Added to the description.', false);
        ...
        // Or upload one.
        var file = box.querySelector('[data-uc-desc-upload]');
        if (!file) { return; }

        file.addEventListener('change', function () {
            if (!this.files || !this.files.length) { return; }

            var url = box.getAttribute('data-uc-ajax');
            var action = box.getAttribute('data-uc-action');
            var field = box.getAttribute('data-uc-field');
            var nonce = box.getAttribute('data-uc-ajax-nonce');
            if (!url || !action || !field) { return; }

            var data = new FormData();
            data.append('action', action);
            data.append('nonce', nonce || '');
            data.append(field, this.files[0]);
            ...
            fetch(url, { method: 'POST', credentials: 'same-origin', body: data })
                ...
                .then(function (res) {
                    ...
                    if (!res || !res.success || !res.data || !res.data.url) {
                        say((res && res.data && res.data.message) || 'That image could not be saved.', true);
                        return;
                    }
                    if (insert(res.data.url, '')) {
                        // The size note is information, not a refusal: the
                        // picture is in and this says what is worth knowing
                        // about it. Same rule as a submitted picture's warning.
                        say(res.data.warning || 'Added to the description.', false);
                        box.removeAttribute('open');
                    }
                })
                .catch(function () {
                    input.disabled = false;
                    say('That upload did not finish. Try it again.', true);
                });
        });
    }
    /* ---------------------------------------------------------------------
    ...
    /* ---------------------------------------------------------------------
     * THE SAME NARROWING, FROM AN ATTRIBUTE RATHER THAN A SELECT (3.94.0)
     *
     * The event editor uses the request form's picker now instead of wp.media,
     * and its series cannot change on the page: it is a property of the event
     * being edited. So there is no <select> to listen to, and the id arrives on
     * the picker as data-uc-image-series-fixed.
     *
     * EVERYTHING ELSE IS THE SAME MECHANISM ON PURPOSE. It sets
     * data-uc-off-series on the rows it wants gone and asks initFilterLists()
     * to run, exactly as initSeriesImageFilter() does, because one owner of
     * `hidden` is what stopped the search box and the series filter fighting
     * over the same rows. A second hiding scheme here would be that fight
     * again.
     *
     * AND THERE IS A WAY PAST IT, which the public forms do not need and an
     * approver does. Pressing it clears the narrowing for good rather than
     * toggling: somebody who has asked to see everything has answered the
     * question, and a control that puts the filter back is one more thing to
     * understand on a screen that already has enough.
     * ------------------------------------------------------------------ */
    function initFixedSeriesImageFilter() {
        var picker = document.querySelector('[data-uc-image-picker][data-uc-image-series-fixed]');
        if (!picker) { return; }

        var id = String(picker.getAttribute('data-uc-image-series-fixed') || '').trim();
        var scope = picker.querySelector('[data-uc-filter-scope]') || picker;
        var list = picker.querySelector('[data-uc-filter-list]');
        var all = picker.querySelector('[data-uc-image-show-all]');
        var note = picker.querySelector('[data-uc-image-none]');
        if (!list) { return; }
        ...
        /* WITH NO SERIES THERE IS NOTHING TO NARROW TO, so nothing is hidden
         * and the escape is pointless. Same branch the select version has. */
        if (!id || '0' === id) {
        ...
        function apply(on) {
            ...
            narrowed = on;
            /* THE CHOSEN PICTURE IS NEVER HIDDEN. It is what the event is
             * already using, so hiding it would make the panel disagree with
             * the summary row above it, and unticking is the only way back. */
            var picked = list.querySelector('input[type="radio"]:checked');
            if (picked) {
                var row = picked.closest('[data-uc-image-series]');
                if (row && row.hasAttribute('data-uc-off-series')) {
                    row.removeAttribute('data-uc-off-series');
                    shown++;
                }
            }
            if (note) { note.hidden = !(on && 0 === shown); }
     ...
     * ------------------------------------------------------------------- */
    /* ---------------------------------------------------------------------
     * A HYBRID EVENT ALWAYS TAKES RSVPS (3.97.2)
     *
     * The format question, in person or online, is asked on the registration
     * form and nowhere else, so an event with registrations switched off has
     * no way for anybody to answer it, and the two capacities are limits on a
     * question nobody is ever asked.
     *
     * SO BOTH TICKS GO ON AND LOCK while hybrid is ticked, and BOTH COME BACK
     * when it is unticked. What comes back is the stored value rather than a
     * default: only the disabled attribute moves on the way out, so somebody
     * who had registrations off before trying hybrid still has them off after
     * changing their mind.
     *
     * IT IS NOT WHAT MAKES IT TRUE. The save asks the stored format and writes
     * the value either way, so with scripting off, or with the attribute
     * removed by hand, nothing about what is saved changes. This only keeps the
     * screen honest, which is the arrangement initCalendarTick() documents one
     * control along.
     *
     * AND IT MUST NOT UNLOCK WHAT THE THIRD-PARTY RULE LOCKED. An imported
     * event's Accept RSVPs is rendered off and disabled by the server and can
     * never be hybrid, so a box that is already disabled AND unticked is left
     * exactly as it is.
     * ------------------------------------------------------------------ */
    function applyHybridRsvpLock(isHybrid) {
        var accept = document.querySelector('[data-uc-rsvp-accept-check]');
        var show   = document.querySelector('[data-uc-rsvp-show-check]');
        var note   = document.querySelector('[data-uc-rsvp-hybrid-note]');
        var shown  = document.querySelector('[data-uc-rsvp-hybrid-show-note]');
        var plain  = document.querySelector('[data-uc-rsvp-show-note]');

        var pairs = [[accept, note], [show, shown]];
        for (var i = 0; i < pairs.length; i++) {
            ...
            var box = label.querySelector('input');
            if (!box) { continue; }
            if (box.disabled && !box.checked && !isHybrid) { continue; }
            /* THE VALUE IT ARRIVED WITH IS REMEMBERED, ONCE, AND GIVEN BACK.
             *
             * Setting checked on the way in and only moving `disabled` on the
             * way out leaves the box TICKED: somebody whose event took no
             * registrations, who tried hybrid and changed their mind, was left
             * with a registration form they never asked for, and the save
             * writes whatever the box says once hybrid is gone.
             *
             * WRITTEN ONCE, on the first lock. apply() runs on every change of
             * either tick, so storing it unconditionally would overwrite the
             * real answer with `true` the second time hybrid was seen ticked. */
            if (isHybrid) {
            ...
            box.disabled = isHybrid;
            label.classList.toggle('uc-check-locked', isHybrid);
            if (hint) { hint.hidden = !isHybrid; }
        }
    ...
    function initLocationPicker() {
        document.querySelectorAll('[data-uc-location]').forEach(function (root) {
            ...
            var hybrid = root.querySelector('[data-uc-hybrid-toggle]');
            var place  = root.querySelector('[data-uc-location-place]');
            var panel  = root.querySelector('[data-uc-online-panel]');
            var onlineCap = document.querySelector('[data-uc-online-capacity]');
            var capLabel  = document.querySelector('[data-uc-capacity-label]');
            var capInPer  = document.querySelector('[data-uc-capacity-in-person]');

            if (!modes.length && !online) {
                return;
            }

            /* HYBRID IS ONLINE AND IN PERSON, WHICH IS WHY IT SHOWS BOTH HALVES.
             *
             *     online only   the meeting link, no venue and no address
             *     hybrid        the meeting link AND the venue and address,
             *                   with the online capacity beside the link
             *     neither       the venue and address
             *
             * AND THE TWO TICKS ARE EXCLUSIVE. They cannot both be true: an
             * event is in person, online, or hybrid. Ticking one clears the
             * other here, and save_event_from_post() folds a posted pair the
             * same way with hybrid winning, so a form sent with the script
             * blocked cannot store a state mode() would have to invent an
             * answer for. */
            function exclusive(changed) {
                if (!online || !hybrid) { return; }
                if (changed === online && online.checked) { hybrid.checked = false; }
                if (changed === hybrid && hybrid.checked) { online.checked = false; }
            }

            /* THE LINK GOES OUT BY DEFAULT (3.98.0).
             *
             * A meeting link nobody is sent is the one shape of this feature
             * that helps nobody, and it was what an event switched to online
             * started in: the organizer entered a link, saved, and the
             * confirmation still said one would be sent before the event.
             *
             * ON THE TRANSITION, NOT ON THE STATE. This fires only when a
             * format tick goes from off to ON, so somebody who unticks a
             * delivery box while online stays ticked keeps it unticked: apply()
             * runs on every change and a rule written against the state would
             * re-tick it under their hands. Switching the format off and on
             * again is a fresh decision and does set them again.
             *
             * THE SERVER HAS THE OTHER HALF, a new event rendering both ticked,
             * so this is not what makes the default true and a blocked script
             * loses nothing on the case that matters most. */
            var wasOnAFormat = !!((online && online.checked) || (hybrid && hybrid.checked));
            function defaultDeliveries() {
                var nowOn = !!((online && online.checked) || (hybrid && hybrid.checked));
                if (nowOn && !wasOnAFormat) {
                    Array.prototype.forEach.call(
                        document.querySelectorAll('input[name="meeting_send[]"]'),
            ...
            function apply() {
                var isHybrid = !!(hybrid && hybrid.checked);
                var isOnline = !!(online && online.checked);

                /* The link panel belongs to both formats that have a link. */
                if (panel) { panel.hidden = !(isOnline || isHybrid); }
                /* The place is hidden only by a PURELY online event. */
                if (place) { place.hidden = isOnline && !isHybrid; }
                /* The online capacity is a hybrid control: a non-hybrid event's
                 * single limit is the box under the address. */
                /* CAPACITY IS ONE ROW, AND HYBRID IS WHAT PUTS TWO BOXES IN IT
                 * (3.98.0). With one box the row label "Capacity" is the whole
                 * name, so the format names appear only when there are two of
                 * them to tell apart. The in-person input's accessible name
                 * moves with them: a visible label that comes and goes is no
                 * use to somebody who is not looking at it. */
                if (onlineCap) { onlineCap.hidden = !isHybrid; }
                if (capLabel)  { capLabel.hidden  = !isHybrid; }
                if (capInPer)  { capInPer.setAttribute('aria-label', isHybrid ? 'Capacity, in person' : 'Capacity'); }
            ...
            if (online) {
                online.addEventListener('change', function () { exclusive(online); defaultDeliveries(); apply(); });
            }
            if (hybrid) {
                hybrid.addEventListener('change', function () { exclusive(hybrid); defaultDeliveries(); apply(); });
            }
    ...
    /* ---------------------------------------------------------------------
     * REQUIRED MARKS THAT MOVE WITH THEIR CONDITION (3.99.0)
     *
     * The two public forms draw every asterisk from one list per form, the
     * same list their validator refuses from. Most entries never change, and
     * the server draws those and is done. The ones handed here in
     * `script[data-uc-required]` are the ones that can: a box required only
     * once "Something else" is chosen, a contact required only while "use my
     * details" is off, a street required only while no venue is picked. This
     * re-reads those entries whenever the form changes.
     *
     * IT ONLY SHOWS AND HIDES WHAT THE SERVER ALREADY DREW, and it only ever
     * sets what the entry says the control carries: `required` on a control
     * that can take it, aria-required on one that cannot. With this script
     * missing, each mark and attribute stays as the server drew it for the
     * answers on the page, and the validator is still the check.
     *
     * A SELECT READING "0" IS EMPTY, as the server reads it: that is the venue
     * select's "enter it by hand". A "0" typed into a box is an answer.
     * ------------------------------------------------------------------- */
    function initRequiredMarks() {
        document.querySelectorAll('script[data-uc-required]').forEach(function (node) {
            var form = node.closest('form');
            if (!form) { return; }
            var list;
            try { list = JSON.parse(node.textContent || '[]'); } catch (err) { return; }
            ...
            function valueOf(name) {
                ...
                form.querySelectorAll('[name="' + name + '"]').forEach(function (el) {
                    if ('' !== out) { return; }
                    /* A checkbox carries its value ticked or not; `checked`
                     * is the only thing that answers for one. */
                    if ('checkbox' === el.type || 'radio' === el.type) {
                        if (el.checked) { out = String(el.value); }
                        return;
                    }
                    out = String(el.value || '').trim();
                    if ('SELECT' === el.tagName && '0' === out) { out = ''; }
                });
                return out;
            }
            ...
            function sync() {
                list.forEach(function (entry) {
                    var when = entry.when || {};
                    var on = Object.keys(when).every(function (name) {
                        return valueOf(name) === String(when[name]);
                    });
                    form.querySelectorAll('[data-uc-req-mark="' + entry.key + '"], [data-uc-req-said="' + entry.key + '"]').forEach(function (m) {
                        m.hidden = !on;
                    });
                    (entry.inputs || []).forEach(function (name) {
                        form.querySelectorAll('[name="' + name + '"]').forEach(function (el) {
                            if ('required' === entry.carry) {
                                el.required = on;
                            } else if ('aria' === entry.carry) {
        ...
        /* A REQUIRED RICH TEXT BOX SAYS SO WHERE THE READER ACTUALLY IS. The
         * textarea carries aria-required, but TinyMCE hides it and the cursor
         * is in the editor's own body. Editors can start before or after this
         * runs, so both are covered. */
        if (!window.tinymce) { return; }
        function mirror(ed) {
            try {
                var area = ed.getElement && ed.getElement();
                var body = ed.getBody && ed.getBody();
                if (area && body && 'true' === area.getAttribute('aria-required')) {
                    body.setAttribute('aria-required', 'true');
                }
            } catch (err) { /* the textarea still says it */ }
        }
        function watch(ed) {
            if (!ed || typeof ed.on !== 'function') { return; }
            if (ed.initialized) { mirror(ed); } else { ed.on('init', function () { mirror(ed); }); }
        }
        var started = window.tinymce.editors || [];
        for (var i = 0; i < started.length; i++) { watch(started[i]); }
        if (typeof window.tinymce.on === 'function') {
            try {
                window.tinymce.on('AddEditor', function (e) { watch(e && e.editor); });
    ...
    function initSeriesPrefill() {
        ...
        var OPTIONS = [
            ...
            },
            {
                ...
                preview: function (d) { return d.start_time + (d.end_time ? ' to ' + d.end_time : ''); },
                /*
                 * THE HOUR AND MINUTE PAIRS (3.104.0). Since 3.98.0 a time is
                 * two selects, <name>_h and <name>_m, and there is no field
                 * named start_time on the form, so this read and wrote nothing
                 * and Fill these in silently left the times empty. A stored
                 * minute that is not on the five-minute list is added as its
                 * own option, the server's "(current)" rule, rather than
                 * rounded.
                 */
                filled: function () {
                    var s = form.querySelector('[name="start_time_h"]');
                    var e = form.querySelector('[name="end_time_h"]');
                    return (s && s.value !== '') || (e && e.value !== '');
                },
                write: function (d) {
                    [['start_time', d.start_time], ['end_time', d.end_time]].forEach(function (pair) {
                        var h = form.querySelector('[name="' + pair[0] + '_h"]');
                        var m = form.querySelector('[name="' + pair[0] + '_m"]');
                        var parts = String(pair[1] || '').split(':');
                        if (!h || !m) { return; }
                        if (parts.length < 2) { h.value = ''; m.value = ''; return; }
                        /* The hour options are zero-padded, "09" not "9". */
                        h.value = ('0' + parseInt(parts[0], 10)).slice(-2);
                        var mm = parts[1].slice(0, 2);
                        if (!m.querySelector('option[value="' + mm + '"]')) {
                            var opt = document.createElement('option');
                            opt.value = mm;
                            opt.textContent = ':' + mm + ' (current)';
                            m.appendChild(opt);
                        }
                        m.value = mm;
