...
**Publish no longer refuses a complete event, the times on both public forms work again, a typed image URL is the event's own picture, and the pending bar's three setters are one panel with one Apply.**

**PUBLISH REFUSED A COMPLETE EVENT, AND THE TIMES HAD NOT SAVED SINCE 3.98.0.** With both times set, Publish answered "Saved, and not published. Add a start time and an end time, then publish." Reproduced in headless Chrome against the real editor and portal.js: the browser posts each time as its two halves, `start_time_h` and `start_time_m`, correctly. They are folded back into `start_time` on `init` by `sfaf_normalize_time_post()`, and WordPress calls an action that carries no arguments with an empty string, so the function received `''` where it expected its list of names and folded nothing. It was introduced in 3.98.0 with the hour and minute controls; the 3.99.0 publish rule made it visible. On a real request since then a new event saved no times and was held, an existing event kept its stored times and quietly dropped a change, and **both public forms refused every submission with "Give a start time."** The function now takes anything that is not a list of names as its default list. Every earlier check called it by hand with no argument, the one call WordPress never makes.

**THE SERIES PREFILL FILLS THE TIMES AGAIN.** Its "Fill these in" wrote the times into a field named `start_time` that has not existed since 3.98.0, so it did nothing for them. It writes the hour and minute now, adding a stored minute that is off the five-minute list rather than rounding it.

**A TYPED IMAGE URL WINS OVER THE SERIES PICTURE, WHEREVER IT POINTS.** Mark's decision, reversing 3.83.0 for this one box: a URL typed into the editor's "Or an image URL" box, which the pending row's image control shares, is the event's own picture even inside uploads and outside the calendar folder. The save records what was typed, and the folder rule exempts the stored URL only while it is still exactly that, so the imported pictures the rule was made for, and anything else written there, still answer to it. Attachments are unchanged. The editor's "Event-specific" tag asks the same function every surface does, so the two cannot disagree.

**CHECKED FOR THESE TWO.** A new browser check sets the times through the controls in Chrome, presses the real Publish, Save changes and Save draft, and runs what the browser posted through the hook the way WordPress runs it: a complete new event publishes, an existing one keeps its new times on Save changes, a start with no end is held for the end time alone, Save draft stores both, and both public forms read both times. Approve reads stored times and was unaffected. A second test saves a typed URL through the real save and reads the event page, both cards, the month tile's hover preview and the pending row. Planting the old signature fails the first with the exact message that was reported; planting the typed URL losing again fails the second.

**THE PENDING BAR IS ONE PANEL, ONE PRESS, ONE SAVE PER ROW.** Series, Categories and Organizers sit side by side, with a single Apply under them. 3.103.0 gave each its own button, so setting all three on forty rows was three presses. Apply writes every control that has a value and leaves a blank one alone: a blank control never clears anything, and there is no way to empty a field from the panel. Ticked categories and organizers replace what each row has.
