...
**Last updated:** 2026-08-28, at 3.62.1.

---

## What shipped last

**3.62.1**, built as `sfaf-calendar-3.62.1.zip` in the project root, committed
...
| | |
|---|---|
| **3.62.0**, and **3.62.1** | Online events. One tick, "Online Event" everywhere a location renders, a meeting link that goes out only with the messages the manager ticks, and a whitelist that fails the build if anything else reads it. **3.62.1 is copy only**: the four hints under the tick described the implementation rather than what happens to the event, which is the standing fault in `CLAUDE.md` §6. |
...
repo.** The tell is the Plugins screen: if it does not say 3.62.1, the
